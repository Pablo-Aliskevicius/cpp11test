...
#ifdef _MSC_VER
                cout << setw(60) << name << ": " << testCount << " calls in " << setw(10) << elapsed << Reset << " nanoseconds." << endl;
#else
...
#ifdef _MSC_VER
        results.insert(result(InlinedArrayLookup(tests), " Potentially inlined array lookup"));
        results.insert(result(InlinedStandardLoop(tests), " Potentially inlined standard loop"));
        results.insert(result(InlinedMatrix(tests), " Potentially inlined matrix potentiation"));
        results.insert(result(InlinedWithoutLoopsOrRecursion(tests), " Potentially inlined floating point version"));
        results.insert(result(InlinedRecursion(tests), " Potentially inlined recursion"));
#else
        cout << "\033[1;37;40m";
